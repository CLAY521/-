...
template <class T> class gzList 
{
	...
	public:
		...
		T	*findSortItem(const gzDouble &sortval,const T *prevItem=NULL);
...
public:
	...
	gzDict(gzUInt32 size=10U,gzBool useAutoResize=FALSE);
...
template <class T1 , class T2> inline gzDict<T1,T2>::gzDict(gzUInt32 size,gzBool useAutoResize)
{
	m_array.m_owner=this;
	m_size=size;
	if(m_size)
...
template <class T1 , class T2> inline gzBool gzDict<T1,T2>::exist(const T1 &keyItem ,T2 *valueItem) const
{
	...
	gzDictEntry<T1,T2> *prevEntry=NULL;
...
template <class T> inline gzLateBindArray<T>::gzLateBindArray(gzUInt32 size):m_pointer(NULL),m_size(size){};
...
{
	gzArray<T>::setSize(x_size*y_size);
