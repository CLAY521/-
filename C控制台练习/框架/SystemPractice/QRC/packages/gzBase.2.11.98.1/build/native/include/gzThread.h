...
public:
	...
	GZ_BASE_EXPORT	static gzVoid 	gzDelayFunction( gzUInt32 milliseconds , gzEvent *event=NULL);
...
public:
	...
	GZ_BASE_EXPORT	gzBool wait(gzUInt32 timeout,gzUInt32 fences,gzBool anyAdd=FALSE,gzBool anyRemove=FALSE,gzUInt32 *add_id=NULL,gzUInt32 *remove_id=NULL);
...
{
	GZ_BATCH_WORK_TYPE_BALANCED,
	GZ_BATCH_WORK_TYPE_UNIFORM,
};
...
public:
	...
	GZ_BASE_EXPORT	gzBool addWork(gzBatchWork *work,gzBool triggerWork=TRUE,gzBatchWorkType addedWork= GZ_BATCH_WORK_TYPE_UNIFORM ,gzBatchFence *external_fence=NULL, gzUInt32 pool=0xFFFFFFFFUL);

	GZ_BASE_EXPORT	gzVoid triggerWork();

	GZ_BASE_EXPORT	gzVoid clearWork();

	GZ_BASE_EXPORT	gzBool wait(gzUInt32 timeout , gzUInt32 count=0xFFFFFFFFUL);
