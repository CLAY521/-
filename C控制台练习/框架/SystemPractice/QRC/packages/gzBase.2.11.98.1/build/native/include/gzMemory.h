...
const gzUInt32 GZ_MEM_DEFAULT_STATE = 1;

//! Internal used by GizmoSDK utilities
const gzUInt32 GZ_MEM_INTERNAL_STATE = 99999;
...
#ifdef GZ_ADVANCED_MEM	// Newer compilers
...
#else
	#define GZ_ALLOC_TEST(x) if(!(x)) gzMemoryControl::memprintf(FALSE,"Bad Alloc:%s", GZ_DEBUG_INFO(GZ_VERSION_STR));
#endif
...
{
public:
	gzMemOptimizer(gzUInt32 slots , gzUInt32 byteSize,const char *name=NULL,gzBool enable=FALSE);
