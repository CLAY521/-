...
public:		
	...
	gzBool			contains(const gzString & substr, gzUInt16 startindex=0 , gzBool caseSensitive = TRUE , gzUInt16 *index=NULL) const;
	...
	gzBool			compare( const gzString &str , gzBool caseSensitive = TRUE , gzUInt16 *count=NULL) const;
	...
	*$		takes argument and uses that value as argument position 1-16
	...
	gzString		convertCtrlToText(gzBool changeSpace=FALSE) const;
	...
	gzInt64		dotValue(gzUInt32 base=256,gzUInt32 *groups=NULL) const; // e.g. "255.234.245.23"
...
GZ_BASE_EXPORT gzVoid gzLtoA( char * buffer , gzInt64 number , gzUByte radix , gzInt16 precision=0 , gzUInt16 *len=NULL , gzBool upper=FALSE);

//! Generates an unicode representation of the long integer number with radix 
GZ_BASE_EXPORT gzVoid gzLtoA( gzWideChar * buffer , gzInt64 number , gzUByte radix , gzInt16 precision=0 , gzUInt16 *len=NULL , gzBool upper=FALSE);

//! Generates an ascii representation of the long integer number with radix 
GZ_BASE_EXPORT gzVoid gzULtoA( char * buffer , gzUInt64 number , gzUByte radix , gzInt16 precision=0 , gzUInt16 *len=NULL , gzBool upper=FALSE);

//! Generates an unicode representation of the long integer number with radix 
GZ_BASE_EXPORT gzVoid gzULtoA( gzWideChar * buffer , gzUInt64 number , gzUByte radix , gzInt16 precision=0 , gzUInt16 *len=NULL , gzBool upper=FALSE);

//! Generates an ascii identifier representation of the unsigned long integer number
GZ_BASE_EXPORT gzVoid gzULtoIdent( char * buffer , gzUInt32 maxlen, gzUInt64 number , gzInt16 precision=0 , gzUInt16 *len=NULL );

//! Generates an ascii representation of the double precision number
GZ_BASE_EXPORT gzVoid gzDtoA( char * buffer , gzDouble number , gzInt16 precision=6 , gzUInt16 *len=NULL , gzInt32 digits=15);

//! Generates an unicode representation of the double precision number
GZ_BASE_EXPORT gzVoid gzDtoA( gzWideChar * buffer , gzDouble number , gzInt16 precision=6 , gzUInt16 *len=NULL , gzInt32 digits=15);
...
GZ_BASE_EXPORT gzUInt32 gzUTF8ToULong(const char *adress,gzUByte *size=NULL);
...
public:
	...
	static gzBool			setDefaultRegistry(const gzString &file_url, gzString password=GZ_EMPTY_STRING, gzString* errorString=NULL, gzSerializeAdapterError* errorType=NULL);
	static const gzString &	getDefaultRegistry();

	// ------------ local settings --------------------
	static gzBool	setLocalRegistry(const gzString &url, gzString password=GZ_EMPTY_STRING, gzString* errorString=NULL, gzSerializeAdapterError* errorType=NULL,gzBool useChecksum=FALSE);
...
public:
	...
	GZ_BASE_EXPORT gzUInt32	getDataSize(gzSerializeAdapter *adapter=NULL) const;
