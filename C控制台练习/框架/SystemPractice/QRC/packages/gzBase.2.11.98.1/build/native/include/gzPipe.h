...
public:
	...
	GZ_BASE_EXPORT virtual gzBool	onPipeOpenRequest(const gzString & /*pipeID*/ , const gzString & /*optionString*/)	{ return TRUE; }
	GZ_BASE_EXPORT virtual gzBool	onPipeOpenAck(const gzString & /*pipeID*/ , const gzString & /*optionString*/)		{ return TRUE; }
};
...
public:
	...
	GZ_BASE_EXPORT gzPipe(gzPipeControlInterface *pipeControl=NULL);
	...
	GZ_BASE_EXPORT static gzString queryNamedPipe(const gzString & address = GZ_EMPTY_STRING, gzUInt32 retry = 10,gzBool extended=FALSE);
...
protected:
	...
	GZ_BASE_EXPORT virtual gzBool	onPipeOpenRequest(const gzString &pipeID , const gzString &optionString) override;
	GZ_BASE_EXPORT virtual gzBool	onPipeOpenAck(const gzString &pipeID , const gzString &optionString) override;
...
public:
	...
	GZ_DECLARE_SERIALIZE_3("SaveFile", url,flags,password);
};
...
GZ_BASE_EXPORT gzBool gzReceivePurlPayload(gzSerializeAdapter* pipe, gzSerializeAdapter* adapter, gzUInt32 &data_payload_length, gzUInt32 retry = 50, gzString* errorString = NULL, gzString* progressString=NULL);
...
GZ_BASE_EXPORT gzDynamicType	gzReceiveTopic(gzSerializeAdapter* adapter, const gzGUID& id, gzArray<gzUByte>* payload = NULL, gzPurlMux *mux=NULL, gzUInt32 retry = 50, gzDouble timeOut = 10 , gzExpectedPurlCommandBits expect=GZ_EXPECT_PURL_COMMAND_ALL);
GZ_BASE_EXPORT gzDynamicType	gzQueryTopic(gzSerializeAdapter* adapter, const gzString& topic, const gzDynamicType& parameters = gzDynamicTypeVoid(), const gzGUID& id = gzGUID::generateGUID(), gzArray<gzUByte>* payload = NULL, gzPurlMux *mux=NULL, gzUInt32 retry = 50, gzDouble timeOut = 10);
