...
{	
	...
	GZ_CLONE_SHARE_ALL			= 0xFFFFFFFFUL, 
};
...
public:
	...
	virtual gzVoid	setCopyMode(const gzCopyMode & mode) { m_copyMode=mode; }
...
inline gzReference::~gzReference()
{
	if(_refCount)
	{
		throwFatalException("REFERENCE MISMATCH ! Deleted referenced object with nonzero reference count");
	}
}
...
{
	GZ_EXCEPTION(throwFatalException("Unimplemented clone called for type '%s'", (const char*)getTypeName()));
...
inline gzVoid gzReference::ref()
{
	if(_refCount<0xFFFFFFFFUL)
		...
		GZ_EXCEPTION(throwFatalException("Too many ref called for type '%s'", (const char*)getTypeName());)
}
...
{
public:
	gzRefDict(gzUInt32 size=10U,gzBool useAutoResize=FALSE):gzDict<T1,T2>(size,useAutoResize),m_useDelayedRemove(FALSE){};
...
public:
	...
	virtual gzReference* clone() const 
	{ 
		return new gzRefData<T1,T2>(*this); 
	}
