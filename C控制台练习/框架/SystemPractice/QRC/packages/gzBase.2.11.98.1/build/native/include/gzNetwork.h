...
public:
	...
	*/
	...
	GZ_BASE_EXPORT	gzInt32 send(const gzUByte *buffer,gzUInt32 len);

	//! Send datagram to a specific destination address
	GZ_BASE_EXPORT	gzInt32 sendTo(const gzSocketAddress & address ,const gzUByte *buffer,gzUInt32 len);
	...
	*/
	...
	//! Set the last socket error
	GZ_BASE_EXPORT	gzVoid setSocketError(gzSocketError error=GZ_SOCKET_ERROR_CHECK_FOR_ERROR);
