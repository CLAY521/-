...
const gzString	GZ_URLBASE_FILE				= "file:";	// path/file name?append=true/false
...
const gzString	GZ_URLBASE_ZIP				= "zip:";	// {url},subfile
...
public:
	...
	GZ_BASE_EXPORT virtual	gzBool		canSeek() { return FALSE; }
	//! Set seekmode to TRUE if you intend to use seek
	GZ_BASE_EXPORT virtual	gzVoid		setSeekMode(gzBool /*on*/)	{ };

	GZ_BASE_EXPORT virtual	gzMemSize	seek(gzMemOffset /*offset*/, gzOriginPos /*origin*/=GZ_ORIGIN_SET){ return (gzMemSize)-1; }
	...
	//! Positions 
	GZ_BASE_EXPORT const gzUInt64& getReadPosition() const	{ return m_readPosition; }
	GZ_BASE_EXPORT const gzUInt64& getWritePosition() const { return m_writePosition; }

	//--- The VERY generic URL based adaptor factory ----------------------------------------------------

	GZ_BASE_EXPORT static gzSerializeAdapter *getURLAdapter(const gzString &url,gzSerializeAction action=GZ_SERIALIZE_INPUT,gzSerializeAdapterFlags flags=GZ_SERIALIZE_ADAPTER_FLAGS_DEFAULT, const gzString &password=GZ_EMPTY_STRING,gzString *errorString=NULL, gzSerializeAdapterError *errorType=NULL);
	...
	GZ_BASE_EXPORT static gzSerializeURLManagerInterface *getURLManager(const gzString &urlBase,gzSerializeURLManagerInterface *prev=NULL);

	//! Returns a URL encoding. Forward Slashes
	/*!
		\return TRUE means that the URL is an Absolute path
	*/
	GZ_BASE_EXPORT static gzBool	getURLBaseEncoding(const gzString &url, gzString *_urlBase=NULL,gzString * _urlPath=NULL,gzString *_urlName=NULL,gzString *_urlAttributes=NULL,gzBool *_syntax_OK=NULL,gzBool enablePathExtension=TRUE);
...
public:
	...
	GZ_BASE_EXPORT gzVoid reset(gzVoid *adress=NULL,gzUInt32 maxlen=0);
...
public:
	...
	GZ_PROPERTY_EXPORT(gzBool,	Active,		GZ_BASE_EXPORT);
...
public:
	...
	GZ_BASE_EXPORT virtual gzUInt32	getDataSize(gzSerializeAdapter *adapter=NULL) const =0;
};
...
public:
	...
	virtual gzUInt32	getDataSize(gzSerializeAdapter * /*adapter*/=NULL) 	const override	{ return sizeof(m_data); }
...
public:
	...
	GZ_BASE_EXPORT virtual gzUInt32 getDataSize(gzSerializeAdapter * /*adapter*/ =NULL) const override { return (gzUInt32)(m_data.length()+1); }
...
public:
	...
	GZ_BASE_EXPORT virtual gzUInt32	getDataSize(gzSerializeAdapter * /*adapter*/=NULL) const { return 2 * sizeof(gzFloat); }
...
public:
	...
	GZ_BASE_EXPORT virtual gzUInt32	getDataSize(gzSerializeAdapter * /*adapter*/=NULL) const { return 3 * sizeof(gzFloat); }
...
public:
	...
	GZ_BASE_EXPORT virtual gzUInt32 getDataSize(gzSerializeAdapter * /*adapter*/ =NULL) const override { return sizeof(gzGUID); }
...
public:
	...
	GZ_BASE_EXPORT virtual gzUInt32	getDataSize(gzSerializeAdapter * /*adapter*/=NULL) const 
	{ 
...
public:
	...
	virtual gzUInt32	getDataSize(gzSerializeAdapter * /*adapter*/=NULL) const override
	{
		if(m_useSizeInfo)
		{
			gzVarUInt64 size;
...
public:
	...
	GZ_PROPERTY_EXPORT(gzString,					PurlName,		GZ_BASE_EXPORT);
...
public:
	...
	GZ_BASE_EXPORT gzSerializeAdapterString(const gzString *data=NULL);
...
private:
	...
	public:
		...
		gzMessageDatabaseEntry(const char *message=NULL);
...
#define GZ_DECLARE_SERIALIZE_3(type,x0,x1,x2) gzVoid write(gzSerializeAdapter *adapter) { writeAdapter(adapter,x0);writeAdapter(adapter,x1);writeAdapter(adapter,x2);}gzVoid read(gzSerializeAdapter *adapter){readAdapter(adapter,x0);readAdapter(adapter,x1);readAdapter(adapter,x2);}gzVoid pushBack(gzSerializeAdapter *adapter){pushBackAdapter(adapter,x2);pushBackAdapter(adapter,x1);pushBackAdapter(adapter,x0);}gzUInt32	getDataSize(gzSerializeAdapter *adapter=NULL) const{gzUInt32 size(0);size+=::getDataSize(adapter,x0);size+=::getDataSize(adapter,x1);size+=::getDataSize(adapter,x2);return size;}static gzString getDataTag() { return type; }
#define GZ_DECLARE_SERIALIZE_2(type,x0,x1) gzVoid write(gzSerializeAdapter *adapter) { writeAdapter(adapter,x0);writeAdapter(adapter,x1);}gzVoid read(gzSerializeAdapter *adapter){readAdapter(adapter,x0);readAdapter(adapter,x1);}gzVoid pushBack(gzSerializeAdapter *adapter){pushBackAdapter(adapter,x1);pushBackAdapter(adapter,x0);}gzUInt32	getDataSize(gzSerializeAdapter *adapter=NULL) const{gzUInt32 size(0);size+=::getDataSize(adapter,x0);size+=::getDataSize(adapter,x1);return size;}static gzString getDataTag() { return type; }
#define GZ_DECLARE_SERIALIZE_1(type,x0) gzVoid write(gzSerializeAdapter *adapter) { writeAdapter(adapter,x0);}gzVoid read(gzSerializeAdapter *adapter){readAdapter(adapter,x0);}gzVoid pushBack(gzSerializeAdapter *adapter){pushBackAdapter(adapter,x0);}gzUInt32	getDataSize(gzSerializeAdapter *adapter=NULL) const{gzUInt32 size(0);size+=::getDataSize(adapter,x0);return size;}static gzString getDataTag() { return type; }

// Little endian versions

#define GZ_DECLARE_SERIALIZE_10_LE(type,x0,x1,x2,x3,x4,x5,x6,x7,x8,x9) gzVoid write(gzSerializeAdapter *adapter) { writeAdapter(adapter,x0,FALSE);writeAdapter(adapter,x1,FALSE);writeAdapter(adapter,x2,FALSE);writeAdapter(adapter,x3,FALSE);writeAdapter(adapter,x4,FALSE);writeAdapter(adapter,x5,FALSE);writeAdapter(adapter,x6,FALSE);writeAdapter(adapter,x7,FALSE);writeAdapter(adapter,x8,FALSE);writeAdapter(adapter,x9,FALSE);}gzVoid read(gzSerializeAdapter *adapter){readAdapter(adapter,x0,FALSE);readAdapter(adapter,x1,FALSE);readAdapter(adapter,x2,FALSE);readAdapter(adapter,x3,FALSE);readAdapter(adapter,x4,FALSE);readAdapter(adapter,x5,FALSE);readAdapter(adapter,x6,FALSE);readAdapter(adapter,x7,FALSE);readAdapter(adapter,x8,FALSE);readAdapter(adapter,x9,FALSE);}gzVoid pushBack(gzSerializeAdapter *adapter){pushBackAdapter(adapter,x9,FALSE);pushBackAdapter(adapter,x8,FALSE);pushBackAdapter(adapter,x7,FALSE);pushBackAdapter(adapter,x6,FALSE);pushBackAdapter(adapter,x5,FALSE);pushBackAdapter(adapter,x4,FALSE);pushBackAdapter(adapter,x3,FALSE);pushBackAdapter(adapter,x2,FALSE);pushBackAdapter(adapter,x1,FALSE);pushBackAdapter(adapter,x0,FALSE);}gzUInt32	getDataSize(gzSerializeAdapter *adapter=NULL) const{gzUInt32 size(0);size+=::getDataSize(adapter,x0,FALSE);size+=::getDataSize(adapter,x1,FALSE);size+=::getDataSize(adapter,x2,FALSE);size+=::getDataSize(adapter,x3,FALSE);size+=::getDataSize(adapter,x4,FALSE);size+=::getDataSize(adapter,x5,FALSE);size+=::getDataSize(adapter,x6,FALSE);size+=::getDataSize(adapter,x7,FALSE);size+=::getDataSize(adapter,x8,FALSE);size+=::getDataSize(adapter,x9,FALSE);return size;}static gzString getDataTag() { return type; }
#define GZ_DECLARE_SERIALIZE_9_LE(type,x0,x1,x2,x3,x4,x5,x6,x7,x8) gzVoid write(gzSerializeAdapter *adapter) { writeAdapter(adapter,x0,FALSE);writeAdapter(adapter,x1,FALSE);writeAdapter(adapter,x2,FALSE);writeAdapter(adapter,x3,FALSE);writeAdapter(adapter,x4,FALSE);writeAdapter(adapter,x5,FALSE);writeAdapter(adapter,x6,FALSE);writeAdapter(adapter,x7,FALSE);writeAdapter(adapter,x8,FALSE);}gzVoid read(gzSerializeAdapter *adapter){readAdapter(adapter,x0,FALSE);readAdapter(adapter,x1,FALSE);readAdapter(adapter,x2,FALSE);readAdapter(adapter,x3,FALSE);readAdapter(adapter,x4,FALSE);readAdapter(adapter,x5,FALSE);readAdapter(adapter,x6,FALSE);readAdapter(adapter,x7,FALSE);readAdapter(adapter,x8,FALSE);}gzVoid pushBack(gzSerializeAdapter *adapter){pushBackAdapter(adapter,x8,FALSE);pushBackAdapter(adapter,x7,FALSE);pushBackAdapter(adapter,x6,FALSE);pushBackAdapter(adapter,x5,FALSE);pushBackAdapter(adapter,x4,FALSE);pushBackAdapter(adapter,x3,FALSE);pushBackAdapter(adapter,x2,FALSE);pushBackAdapter(adapter,x1,FALSE);pushBackAdapter(adapter,x0,FALSE);}gzUInt32	getDataSize(gzSerializeAdapter *adapter=NULL) const{gzUInt32 size(0);size+=::getDataSize(adapter,x0,FALSE);size+=::getDataSize(adapter,x1,FALSE);size+=::getDataSize(adapter,x2,FALSE);size+=::getDataSize(adapter,x3,FALSE);size+=::getDataSize(adapter,x4,FALSE);size+=::getDataSize(adapter,x5,FALSE);size+=::getDataSize(adapter,x6,FALSE);size+=::getDataSize(adapter,x7,FALSE);size+=::getDataSize(adapter,x8,FALSE);return size;}static gzString getDataTag() { return type; }
#define GZ_DECLARE_SERIALIZE_8_LE(type,x0,x1,x2,x3,x4,x5,x6,x7) gzVoid write(gzSerializeAdapter *adapter) { writeAdapter(adapter,x0,FALSE);writeAdapter(adapter,x1,FALSE);writeAdapter(adapter,x2,FALSE);writeAdapter(adapter,x3,FALSE);writeAdapter(adapter,x4,FALSE);writeAdapter(adapter,x5,FALSE);writeAdapter(adapter,x6,FALSE);writeAdapter(adapter,x7,FALSE);}gzVoid read(gzSerializeAdapter *adapter){readAdapter(adapter,x0,FALSE);readAdapter(adapter,x1,FALSE);readAdapter(adapter,x2,FALSE);readAdapter(adapter,x3,FALSE);readAdapter(adapter,x4,FALSE);readAdapter(adapter,x5,FALSE);readAdapter(adapter,x6,FALSE);readAdapter(adapter,x7,FALSE);}gzVoid pushBack(gzSerializeAdapter *adapter){pushBackAdapter(adapter,x7,FALSE);pushBackAdapter(adapter,x6,FALSE);pushBackAdapter(adapter,x5,FALSE);pushBackAdapter(adapter,x4,FALSE);pushBackAdapter(adapter,x3,FALSE);pushBackAdapter(adapter,x2,FALSE);pushBackAdapter(adapter,x1,FALSE);pushBackAdapter(adapter,x0,FALSE);}gzUInt32	getDataSize(gzSerializeAdapter *adapter=NULL) const{gzUInt32 size(0);size+=::getDataSize(adapter,x0,FALSE);size+=::getDataSize(adapter,x1,FALSE);size+=::getDataSize(adapter,x2,FALSE);size+=::getDataSize(adapter,x3,FALSE);size+=::getDataSize(adapter,x4,FALSE);size+=::getDataSize(adapter,x5,FALSE);size+=::getDataSize(adapter,x6,FALSE);size+=::getDataSize(adapter,x7,FALSE);return size;}static gzString getDataTag() { return type; }
#define GZ_DECLARE_SERIALIZE_7_LE(type,x0,x1,x2,x3,x4,x5,x6) gzVoid write(gzSerializeAdapter *adapter) { writeAdapter(adapter,x0,FALSE);writeAdapter(adapter,x1,FALSE);writeAdapter(adapter,x2,FALSE);writeAdapter(adapter,x3,FALSE);writeAdapter(adapter,x4,FALSE);writeAdapter(adapter,x5,FALSE);writeAdapter(adapter,x6,FALSE);}gzVoid read(gzSerializeAdapter *adapter){readAdapter(adapter,x0,FALSE);readAdapter(adapter,x1,FALSE);readAdapter(adapter,x2,FALSE);readAdapter(adapter,x3,FALSE);readAdapter(adapter,x4,FALSE);readAdapter(adapter,x5,FALSE);readAdapter(adapter,x6,FALSE);}gzVoid pushBack(gzSerializeAdapter *adapter){pushBackAdapter(adapter,x6,FALSE);pushBackAdapter(adapter,x5,FALSE);pushBackAdapter(adapter,x4,FALSE);pushBackAdapter(adapter,x3,FALSE);pushBackAdapter(adapter,x2,FALSE);pushBackAdapter(adapter,x1,FALSE);pushBackAdapter(adapter,x0,FALSE);}gzUInt32	getDataSize(gzSerializeAdapter *adapter=NULL) const{gzUInt32 size(0);size+=::getDataSize(adapter,x0,FALSE);size+=::getDataSize(adapter,x1,FALSE);size+=::getDataSize(adapter,x2,FALSE);size+=::getDataSize(adapter,x3,FALSE);size+=::getDataSize(adapter,x4,FALSE);size+=::getDataSize(adapter,x5,FALSE);size+=::getDataSize(adapter,x6,FALSE);return size;}static gzString getDataTag() { return type; }
#define GZ_DECLARE_SERIALIZE_6_LE(type,x0,x1,x2,x3,x4,x5) gzVoid write(gzSerializeAdapter *adapter) { writeAdapter(adapter,x0,FALSE);writeAdapter(adapter,x1,FALSE);writeAdapter(adapter,x2,FALSE);writeAdapter(adapter,x3,FALSE);writeAdapter(adapter,x4,FALSE);writeAdapter(adapter,x5,FALSE);}gzVoid read(gzSerializeAdapter *adapter){readAdapter(adapter,x0,FALSE);readAdapter(adapter,x1,FALSE);readAdapter(adapter,x2,FALSE);readAdapter(adapter,x3,FALSE);readAdapter(adapter,x4,FALSE);readAdapter(adapter,x5,FALSE);}gzVoid pushBack(gzSerializeAdapter *adapter){pushBackAdapter(adapter,x5,FALSE);pushBackAdapter(adapter,x4,FALSE);pushBackAdapter(adapter,x3,FALSE);pushBackAdapter(adapter,x2,FALSE);pushBackAdapter(adapter,x1,FALSE);pushBackAdapter(adapter,x0,FALSE);}gzUInt32	getDataSize(gzSerializeAdapter *adapter=NULL) const{gzUInt32 size(0);size+=::getDataSize(adapter,x0,FALSE);size+=::getDataSize(adapter,x1,FALSE);size+=::getDataSize(adapter,x2,FALSE);size+=::getDataSize(adapter,x3,FALSE);size+=::getDataSize(adapter,x4,FALSE);size+=::getDataSize(adapter,x5,FALSE);return size;}static gzString getDataTag() { return type; }
#define GZ_DECLARE_SERIALIZE_5_LE(type,x0,x1,x2,x3,x4) gzVoid write(gzSerializeAdapter *adapter) { writeAdapter(adapter,x0,FALSE);writeAdapter(adapter,x1,FALSE);writeAdapter(adapter,x2,FALSE);writeAdapter(adapter,x3,FALSE);writeAdapter(adapter,x4,FALSE);}gzVoid read(gzSerializeAdapter *adapter){readAdapter(adapter,x0,FALSE);readAdapter(adapter,x1,FALSE);readAdapter(adapter,x2,FALSE);readAdapter(adapter,x3,FALSE);readAdapter(adapter,x4,FALSE);}gzVoid pushBack(gzSerializeAdapter *adapter){pushBackAdapter(adapter,x4,FALSE);pushBackAdapter(adapter,x3,FALSE);pushBackAdapter(adapter,x2,FALSE);pushBackAdapter(adapter,x1,FALSE);pushBackAdapter(adapter,x0,FALSE);}gzUInt32	getDataSize(gzSerializeAdapter *adapter=NULL) const{gzUInt32 size(0);size+=::getDataSize(adapter,x0,FALSE);size+=::getDataSize(adapter,x1,FALSE);size+=::getDataSize(adapter,x2,FALSE);size+=::getDataSize(adapter,x3,FALSE);size+=::getDataSize(adapter,x4,FALSE);return size;}static gzString getDataTag() { return type; }
#define GZ_DECLARE_SERIALIZE_4_LE(type,x0,x1,x2,x3) gzVoid write(gzSerializeAdapter *adapter) { writeAdapter(adapter,x0,FALSE);writeAdapter(adapter,x1,FALSE);writeAdapter(adapter,x2,FALSE);writeAdapter(adapter,x3,FALSE);}gzVoid read(gzSerializeAdapter *adapter){readAdapter(adapter,x0,FALSE);readAdapter(adapter,x1,FALSE);readAdapter(adapter,x2,FALSE);readAdapter(adapter,x3,FALSE);}gzVoid pushBack(gzSerializeAdapter *adapter){pushBackAdapter(adapter,x3,FALSE);pushBackAdapter(adapter,x2,FALSE);pushBackAdapter(adapter,x1,FALSE);pushBackAdapter(adapter,x0,FALSE);}gzUInt32	getDataSize(gzSerializeAdapter *adapter=NULL) const{gzUInt32 size(0);size+=::getDataSize(adapter,x0,FALSE);size+=::getDataSize(adapter,x1,FALSE);size+=::getDataSize(adapter,x2,FALSE);size+=::getDataSize(adapter,x3,FALSE);return size;}static gzString getDataTag() { return type; }
#define GZ_DECLARE_SERIALIZE_3_LE(type,x0,x1,x2) gzVoid write(gzSerializeAdapter *adapter) { writeAdapter(adapter,x0,FALSE);writeAdapter(adapter,x1,FALSE);writeAdapter(adapter,x2,FALSE);}gzVoid read(gzSerializeAdapter *adapter){readAdapter(adapter,x0,FALSE);readAdapter(adapter,x1,FALSE);readAdapter(adapter,x2,FALSE);}gzVoid pushBack(gzSerializeAdapter *adapter){pushBackAdapter(adapter,x2,FALSE);pushBackAdapter(adapter,x1,FALSE);pushBackAdapter(adapter,x0,FALSE);}gzUInt32	getDataSize(gzSerializeAdapter *adapter=NULL) const{gzUInt32 size(0);size+=::getDataSize(adapter,x0,FALSE);size+=::getDataSize(adapter,x1,FALSE);size+=::getDataSize(adapter,x2,FALSE);return size;}static gzString getDataTag() { return type; }
#define GZ_DECLARE_SERIALIZE_2_LE(type,x0,x1) gzVoid write(gzSerializeAdapter *adapter) { writeAdapter(adapter,x0,FALSE);writeAdapter(adapter,x1,FALSE);}gzVoid read(gzSerializeAdapter *adapter){readAdapter(adapter,x0,FALSE);readAdapter(adapter,x1,FALSE);}gzVoid pushBack(gzSerializeAdapter *adapter){pushBackAdapter(adapter,x1,FALSE);pushBackAdapter(adapter,x0,FALSE);}gzUInt32	getDataSize(gzSerializeAdapter *adapter=NULL) const{gzUInt32 size(0);size+=::getDataSize(adapter,x0,FALSE);size+=::getDataSize(adapter,x1,FALSE);return size;}static gzString getDataTag() { return type; }
#define GZ_DECLARE_SERIALIZE_1_LE(type,x0) gzVoid write(gzSerializeAdapter *adapter) { writeAdapter(adapter,x0,FALSE);}gzVoid read(gzSerializeAdapter *adapter){readAdapter(adapter,x0,FALSE);}gzVoid pushBack(gzSerializeAdapter *adapter){pushBackAdapter(adapter,x0,FALSE);}gzUInt32	getDataSize(gzSerializeAdapter *adapter=NULL) const{gzUInt32 size(0);size+=::getDataSize(adapter,x0,FALSE);return size;}static gzString getDataTag() { return type; }
...
GZ_BASE_EXPORT gzBool gzSetAssetManagerHandle(gzVoid *handle1,gzVoid *handle2=NULL);
