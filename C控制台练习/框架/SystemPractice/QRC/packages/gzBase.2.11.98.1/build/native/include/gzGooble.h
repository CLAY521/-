...
const gzUInt32	GZ_GOOBLE_DEFAULT_PRECISION_SLOTS = 10;				// approx 25 decimals accuracy (10 bytes)
...
public:
	...
	//GZ_BASE_EXPORT gzGooble root(const gzGooble &pow) const;		// x^(1/pow)

	GZ_BASE_EXPORT gzGooble pow(const gzInt32 &pow) const;		// x^pow
	GZ_BASE_EXPORT gzGooble root(const gzInt32 &pow) const;		// x^(1/pow)
	GZ_BASE_EXPORT gzGooble introot(const gzInt32& pow) const;	// integer x^(1/pow)

	GZ_BASE_EXPORT gzGooble powmod(const gzGooble& pow,const gzGooble mod) const;	// x^(pow) % mod
	...
	GZ_STATIC_PROPERTY_EXPORT(gzUInt32, PrecisionSlots, GZ_BASE_EXPORT);
	GZ_STATIC_PROPERTY_EXPORT(gzBool, ThrowInexactResult, GZ_BASE_EXPORT);

	// -------- Static constructs ----------

	GZ_BASE_EXPORT static gzGooble pow10(gzInt32 pow10);			// 10^pow10
	GZ_BASE_EXPORT static gzGooble pow2(gzInt32 pow2);				// 2^pow2

	GZ_BASE_EXPORT static gzGooble fact(gzInt32 fac);				// x!
