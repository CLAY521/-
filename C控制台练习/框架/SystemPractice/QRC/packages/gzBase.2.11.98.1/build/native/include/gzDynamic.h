...
public:
	...
	GZ_BASE_EXPORT gzBool					asJSON(gzSerializeAdapter * adapter, gzBool formatted = TRUE)		const;
	...
	GZ_BASE_EXPORT static gzDynamicType		fromXML(const gzString &xml,gzBool *ok=NULL,gzBool skipDynTag=TRUE);
	GZ_BASE_EXPORT static gzDynamicType		fromJSON(const gzString &json);
...
public:
	...
	GZ_BASE_EXPORT	gzDynamicType *operator()(gzString *name=NULL);
