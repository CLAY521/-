...
gzBool GZ_BASE_EXPORT checkLexToken(const gzLexTokenDataArray &array , const gzUInt32 argPos , const gzLexDataType & type , gzDynamicType * data=NULL);
...
*/
gzArray<gzString> GZ_BASE_EXPORT parse(const gzString &parseFormatString , const gzString &dataString ,  gzBool *success=NULL,gzUInt16 decimalToken='.');
...
public:
	...
	*/
	GZ_BASE_EXPORT	gzBool		hasOption(const gzString &option,gzBool caseSensitive=FALSE,gzUInt32 *argument=NULL,gzUInt32 *maxArgument=NULL) const;
	...
	GZ_BASE_EXPORT	gzVoid		checkArgumentCount(gzUInt32 required);
...
public:
	...
	gzParseResult parseTokens(const gzString &string)
	{
		...
		if(BaseParser::checkTokens(string.getWideString(len),len))
			return GZ_PARSE_OK;
	...
	gzBool registerRule(const gzString &rule)
	{
		return registerRulesFromString(rule);
	}
	...
	gzString getCompressedIdentifier(const gzString &name,gzBool *created=NULL)
	{
	...
	gzBool registerRulesFromString(const gzString &string)
	{
...
public:
	...
	GZ_BASE_EXPORT	gzVoid				getTokenData(gzUInt32 start,gzUInt16 len,gzArray<gzUByte> &data);
...
public:
	...
	gzParseResult parseItem()						// item	::= SPACE* (PUSH | ACCCEPT | RULE_IDENTIFIER | DQSTRING | QSTRING | SET | HEXITEM | group ) 
	{
...
public:
	...
	GZ_BASE_EXPORT gzStringParser(const gzString &bnf);

	GZ_BASE_EXPORT virtual ~gzStringParser(){};

	GZ_BASE_EXPORT gzArray<gzString> parseArguments(const gzString &rule,gzBool *success=NULL);
