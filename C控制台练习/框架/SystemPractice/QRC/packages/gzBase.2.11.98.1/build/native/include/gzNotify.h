...
public:
	...
	GZ_BASE_EXPORT gzVoid notify(const gzString &command , const gzUInt32 priority =0 , const gzDouble & duration=0.0 , gzTypeInterface *sender=NULL, gzReference *data=NULL , gzUInt32 counter=0,gzBool immediateTrigger=FALSE);
	GZ_BASE_EXPORT gzVoid notify_gzString(const gzString &command , const gzString &data, gzTypeInterface *sender=NULL,gzBool immediateTrigger=FALSE);
