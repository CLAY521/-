...
namespace _4._4取消任务
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var task1 = new Task<int>(()=>TaskMethod("task1",10,cts.Token));
            task1.Start();
            Console.WriteLine($"取消任务前task1的状态:{task1.Status}");
            cts.Cancel();
            Console.WriteLine($"取消如任务后task1的状态:{task1.Status}");
            ...
            cts.Cancel();
            //取消task2之后，task2的状态还是RanToCompletion（已成功完成执行的任务）
            //因为从TPL的角度来说，该任务正常完成了它的工作
            for (int i = 0; i < 5; i++)
            {
                Thread.Sleep(TimeSpan.FromSeconds(0.5));
                Console.WriteLine($"task2的状态:{task2.Status}");
